/*======================================================
	...
	- 사용자 정의 컨트롤러 클래스
	- 직원 데이터 수정 액션 처리
	- 액션 처리 후 employeelist.action 을 다시 요청할 수 있도록 안내
...
public class EmployeeUpdateController implements Controller
{
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			employee.setLunar(Integer.parseInt(lunar));			// check~!!!
			employee.setTelephone(telephone);
			employee.setRegionId(regionId);
			employee.setDepartmentId(departmentId);
			employee.setPositionId(positionId);
			employee.setBasicPay(Integer.parseInt(basicPay));	// check~!!!
			employee.setExtraPay(Integer.parseInt(extraPay));	// check~!!!
